...
public class Ask {
	...
	public void ask(List<NameValuePair> list) {
		try{
			//封装数据到HttpPost
			HttpPost post = new HttpPost(new URI(WebInfo.WEB_ROOT+"appask.action"));
			post.setEntity(new UrlEncodedFormEntity(list, "UTF-8"));
			//开启线程传输数据
			new AskThread(post).start();
		}catch(Exception e){
			e.printStackTrace();
		}
	}
	private class AskThread extends Thread {
		...
		public AskThread(HttpPost post) {
			this.post = post;
		}
